...
## 프로젝트명: internal-tcp-watcher (KEDA Internal Traffic Sensor)
...
#### Phase별 구현 메모 (논의 내용 반영)
- Phase 1: K8s 노드/VM에서 실행, ringbuf 이벤트를 stdout 로깅. 로드 실패, 링버퍼 예약 실패(버퍼 부족), 이벤트 디코딩 실패는 error 로그로 남긴다.
- Phase 2: 감시 네임스페이스는 환경변수(없으면 전체)로 설정, in-cluster config만 사용. ClusterIP + PodIP 매핑(헤드리스/ExternalName 포함)까지 유지하며 맵 용량은 대상 1,000개 기준 여유 잡아 2,048 엔트리, TTL 60초로 관리.
- Phase 3: `/metrics`(0.0.0.0:8080) 고정, 카운터 이름 `internal_tcp_attempts_total`, 레이블은 namespace/service/pod. 스크레이프 주기는 환경변수로 설정, 기본 5초.

#### 구현 진행 메모
- Phase 1: eBPF kprobe(tcp_v4_connect) + ringbuf 로드 및 로그 출력 완료.
- Phase 2: K8s Service/EndpointSlice 매퍼 구현(네임스페이스 env, TTL 60s, 용량 기본 2048, ExternalName DNS 포함). DaemonSet 템플릿/배포 스크립트 추가.
- Phase 3: Prometheus `/metrics` 서버(기본 0.0.0.0:8080)와 카운터 `internal_tcp_attempts_total{destination_namespace,destination_service,destination_pod}` 구현. 매핑 성공 시에만 증가, 빈 라벨은 unknown으로 대체. `METRICS_ADDR`로 포트/주소 오버라이드 가능.
